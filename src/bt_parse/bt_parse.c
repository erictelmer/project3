...
//COMMENT THIS SHIT
void bt_get_tar_file(bt_config_t *config) {
	FILE *f;
	char filew[] = "File: ";
	...
	assert(fgets(line, BT_FILENAME_LEN, f) != NULL);
		
	if (strstr(line,filew) == NULL){
		memset(config->tar_file, 0, BT_FILENAME_LEN);
		return;
	}
	filename = line + strlen(filew);
	strcpy(config->tar_file, filename);
	
	newline = strchr(config->tar_file, '\n');
	assert(newline != NULL);
	*newline = '\0';
}
